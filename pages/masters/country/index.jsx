...
function Index() {
  ...
  const [countryTableQueueData, setCountryTableQueueData] = useState(true);
    ...
    return (
      ...
      searchTerm?.length >= 3 && (
        <div className="searchResults">
          <ul>
            {filteredCountryQueue?.data?.countryMasterFilteredData?.length > 0 ? (
              filteredCountryQueue?.data?.countryMasterFilteredData?.map((results, index) => (
                <li
  ...
  const handleRoute = (countryMasterData) => {
    console.log("🚀 ~ file: index.jsx:221 ~ handleRoute ~ countryMasterData", countryMasterData)
  ...
  return (
    <div className={`${styles.tw_toggle}`}>
      <input
        ...
        value="true"
        checked={data?.Status === 'Approved' ? true : false}
      />
      ...
      <input
        ...
        name={`toggle_${data?._id}`}
        value="false"
        checked={data?.Status === 'Not Approved' ? true : false}
      />
