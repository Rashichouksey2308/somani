...
function Index() {
    ...
    const handleSort = (column) => {
        ...
        dispatch(GetMasterCurrencyQueueRecords(`?page=${currentPage}&limit=${pageLimit}&column=${columnName}&order=${sortOrder}${filterQuery}`));
    };
    useEffect(() => {
        if (window) {
                ...
                <ul>
                    {filteredCurrencyQueue?.data?.currencyFilteredData?.length > 0 ? filteredCurrencyQueue?.data?.currencyFilteredData?.map((results, index) => (
                        <li onClick={() => handleListClose(results)} id={results._id} key={results._id} className="cursor-pointer">
                            ...
                            <span>
                                &nbsp; {appliedFilters?.currencyName === true && <span className='text-right'>{results?.Currency_Name}</span>}
