...
function Index() {
  ...
  const [editTdsSection, setTdsSection] = useState(false);
  const [TdsSectionTableQueueData, setTdsSectionTableQueueData] = useState(true);
  ...
  const handleSort = (column) => {
    ...
    dispatch(
      GetMasterTDSSectionQueueRecords(
    ...
    return (
      ...
      searchTerm?.length >= 3 && (
        <div className="searchResults">
          <ul>
            {filteredTDSSectionQueue?.data?.tdsFilteredData?.length > 0 ? (
              filteredTDSSectionQueue?.data?.tdsFilteredData?.map((results, index) => (
                <li
                ...
                >
                  ...
                  <span>
                    &nbsp;
                    {appliedFilters?.paymentNature === true && (
                      <span className="text-right">{results?.paymentNature}</span>
                    )}
                  </span>
                </li>
              ))
  ...
  const handleRoute = (TDSSectionMasterData) => {
    ...
    setTdsSectionTableQueueData(false);
    setTdsSection(true);
    setTDSSectionMasterDetails({
      tdsId: TDSSectionMasterData._id,
      section: TDSSectionMasterData.section,
      tds_rate: TDSSectionMasterData.tds_rate,
      paymentNature: TDSSectionMasterData.paymentNature,
      createdAt: TDSSectionMasterData.createdAt,
      lastUpdatedBy: TDSSectionMasterData.lastUpdatedBy,
      updatedAt: TDSSectionMasterData.updatedAt,
    });
  };
  const clearState = () => {
    setTDSSectionMasterDetails({
  ...
  const updatedATDate = moment(TDSSectionMasterDetails.updatedAt).format('DD-MMM, hh:mm A');

  return (
    <>
      {TdsSectionTableQueueData === true ? (
            ...
            <div className={`${styles.filter} d-flex align-items-center`}>
              <div className={`${styles.head_header} align-items-center`}>
                ...
                <h1 className={styles.heading}>
                  <span className="text-uppercase">TDS</span> Section Master
                </h1>
              </div>
            ...
            <div className={`${styles.filter}`}>
              <div className={`${styles.head_header} align-items-center`}>
                <img
                  ...
                  alt="ArrowRight"
                  onClick={() => {
                    setTdsSectionTableQueueData(true);
                    setTdsSection(false);
                    clearState();
                  }}
                />
              ...
              <AddNewTDSSection
                editTdsSection={editTdsSection}
                setTDSSectionMasterDetails={setTDSSectionMasterDetails}
                TDSSectionMasterDetails={TDSSectionMasterDetails}
              />
              {editTdsSection && (
