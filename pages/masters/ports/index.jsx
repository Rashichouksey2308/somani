...
function Index() {
    ...
    const handleSort = (column) => {
        ...
        dispatch(GetMasterPortsQueueRecords(`?page=${currentPage}&limit=${pageLimit}&column=${columnName}&order=${sortOrder}${filterQuery}`));
    };
                    ...
                    {filteredPortsQueue?.data?.portFilteredData?.length > 0 ? filteredPortsQueue?.data?.portFilteredData?.map((results, index) => (
                        <li onClick={() => handleListClose(results)} id={results._id} key={results._id} className="cursor-pointer">
                            ...
                            <span>
                                ...
                                &nbsp; {appliedFilters?.status === true && <span className='text-right'>{results?.Approved !== undefined ? results?.Approved : 'Null'}</span>}
                            </span>
                        </li>
...
export const ToggleSwitch = ({ data }) => {
    return (
        <div className={`${styles.tw_toggle}`}>
            <input type="radio" className={styles.toggle_left} name={`toggle_${data?._id}`} value="true" checked={data?.Approved === 'Yes' ? true : false} />
            ...
            <input className={styles.toggle_between} type="radio" name={`toggle_${data?._id}`} value="-1" checked={isUndefined(data?.Approved)} />
            <label className={`${styles.toggle} ${styles.toggle_label_between}`}></label>
            <input type="radio" className={styles.toggle_right} name={`toggle_${data?._id}`} value="false" checked={data?.Approved === 'No' ? true : false} />
