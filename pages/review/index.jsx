...
function Index() {
  ...
  useEffect(() => {
    if (companyData) {
      ...
      table2(statutory, balance, complienceFilter);
    }
    if (Array.isArray(companyData?.error) && companyData?.error?.length > 0) {
      _get(companyData, 'error', [{}]).forEach((item) => {
        handleErrorToast(`${item.description} ${item.message}`);
      });
    }
  ...
  const handleMcaReport = (task) => {
    if (task === 'downlaod') {
      if (mcaReportAvailable) {
        ...
        handleErrorToast('mca report not Available to download');
      }
    } else {
      dispatch(
        McaReportFetch({
  ...
  const primaryBankName = () => {
    ...
    return _get(filteredData[0], 'bankName', '');
  };
  ...
  const toPrintPdf = (
    ...
    const latestYearData = _get(companyData, financialRatioString0, {});
    const previousYearData = _get(companyData, financialRatioString1, {});
            ...
            >
              ...
              {allBuyerList &&
                allBuyerList?.data?.data.map((item, index) => {
                  const name = item?.supplierName?.toUpperCase() ?? 'N A';
                  ...
                  <table width="100%" cellPadding="12" cellSpacing="0" border="0">
                    ...
                    {orderList &&
                      returnFilteredCharges()?.map((charge, index) => {
                        const name = charge?.nameOfChargeHolder;
                        const [fName, lName] = name?.split(' ');
