...
export const GetAllCustomClearance = (payload) => async (dispatch, getState, api) => {
  ...
  try {
    const response = await Axios.get(`${API.corebaseUrl}${API.customClearance}${payload || ''}`, {
      headers: headers,
    });
...
export const GetCustomClearance = (payload) => async (dispatch, getState, api) => {
  dispatch(setIsLoading());
  const cookie = Cookies.get('SOMANI');
  const decodedString = Buffer.from(cookie, 'base64').toString('ascii');
...
export const UpdateCustomClearance = (payload) => async (dispatch, getState, api) => {
  ...
  try {
    const response = await Axios.put(`${API.corebaseUrl}${API.customClearance}`, payload.fd, {
      headers: headers,
    });
    if (response.data.code === 200) {
