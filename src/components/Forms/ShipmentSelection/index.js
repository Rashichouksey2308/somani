...
export default function ShipmentSelection (props) {
  const {
    defaultValue, ref, onChange, selectClass, labelClass, labelName, imageClass, imageName, imageSrc
  } = props
  ...
  return <div className="d-flex">
    ...
    >
      ...
      {listOfOptions.map((element, index) => <option value={element.value} index={index}>{element.name}</option>)}
    </select>
...
const listOfOptions = [{ value: 'shipmentForm', name: '(44A) Shipment From' }, {
