...
const Index = ({ data }) => {
  return (
    <div className={`${styles.tw_toggle}`}>
      ...
      <input className={styles.toggle_between} type="radio" name={`toggle_${data?.createdAt}`} value="-1" checked={isUndefined(data?.profileDetails?.status)} />
      <label className={`${styles.toggle} ${styles.toggle_label_between}`}></label>
      <input type="radio" className={styles.toggle_right} name={`toggle_${data?.createdAt}`} value="false" checked={!isUndefined(data?.profileDetails?.status) && !data?.profileDetails?.status} />
